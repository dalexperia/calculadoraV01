...
const OP = (navigator.userAgent.indexOf('Opera') != -1);
const IE = (navigator.userAgent.indexOf('MSIE') != -1 && !OP);
const GK = (navigator.userAgent.indexOf('Gecko') != -1);
const SA = (navigator.userAgent.indexOf('Safari') != -1);
...
//----------------------------------------------------------------------------------------------------
	...
	this.bgColor = "#dedddd";			// background color
	this.textFont = "Comic Sans MS";	// text font family
	this.textSize = 13;					// text font size (pixels)
	this.textColor = "#2a2a2a";			// text color
	this.textAlign = "center";			// text alignment: "left", "right" or "center"
	this.border = "1px dashed #2a2a2a";	// border (CSS spec: size style color, e.g. "1px solid #D00000")
...
//----------------------------------------------------------------------------------------------------
	...
	this.move = function() {
		const winX = getWinX() - (((GK && !SA) || OP) ? 17 : 0);
		const winY = getWinY() - (((GK && !SA) || OP) ? 17 : 0);
		let x = mouseX;
		let y = mouseY;
...
function getMouseXY(e) {
	...
	if (e && e.pageX != null) {
		...
		mouseX = e.clientX + getScrX();
		mouseY = e.clientY + getScrY();
	}
